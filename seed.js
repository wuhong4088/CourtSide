...
async function runSeed() {
  try {
    ...
    const baseCourts = [
      {
        ...
        review:
          'Excellent outdoor courts with double rims. Lights stay on until 10 PM. Always active with good games.',
        rating: 4.5,
        sport: 'Basketball',
        createdAt: new Date(),
      },
      {
        ...
        review:
          'Newly renovated courts. Clean nets and great surface grip for pickleball and tennis. Easy parking nearby.',
        rating: 5,
        sport: 'Basketball',
        createdAt: new Date(),
      },
      {
        ...
        review:
          'Indoor sports hall with excellent lighting and clean facilities. Hard to reserve peak hours but top quality.',
        rating: 4.8,
        sport: 'Tennis',
        createdAt: new Date(),
      },
      {
        ...
        review:
          'Solid neighborhood court. Rim is slightly low on the south court but clean play area.',
        rating: 3.8,
        sport: 'Basketball',
        createdAt: new Date(),
      },
    ...
    console.log(
      `Seeded ${courts.length} court locations (including >1k synthetic records).`
    );

    console.log(
      'Inserting mock gear checklists (Partner B — Wu Hung Hsiao)...'
    );
    const checklists = [
      {
        userId: 'Morgan',
    ...
    console.log(
      'Inserting synthetic pickup games (Partner A — Harini Thirunavukkarasan)...'
    );
    const games = [];
    ...
    for (let i = 1; i <= 1005; i++) {
      ...
      const maxPlayers =
        sport === 'Tennis' ? 2 : sport === 'Pickleball' ? 4 : 6;
    ...
    console.log(
      `Seeded ${games.length} pickup games (including >1k synthetic records).`
    );

    console.log(
      'Inserting synthetic match results (Partner A — Harini Thirunavukkarasan)...'
    );
    const matches = [];
    ...
    for (let i = 1; i <= 200; i++) {
      ...
      matches.push({
        ...
        userId,
        score:
          outcome === 'WIN' ? `${scoreA} - ${scoreB}` : `${scoreB} - ${scoreA}`,
        outcome,
